...
";
        ...
        protected static Message CreateErrorMessage(string error, string? contextId)
        {
            return new Message
            {
                ...
                Parts = new List<Part> { new TextPart { Text = $"I apologize, but I encountered an error: {error}" } }
            };
        }
    }
...
";
        ...
        public void Attach(ITaskManager taskManager)
        {
            taskManager.OnMessageReceived = async (messageSendParams, ct) =>
            {
                try
                {
                    if (messageSendParams?.Message == null || messageSendParams.Message.Parts == null || !messageSendParams.Message.Parts.OfType<TextPart>().Any())
                    {
                        return BaseA2AAgent.CreateErrorMessage("No valid text content found in message.", messageSendParams?.Message?.ContextId);
                    }
